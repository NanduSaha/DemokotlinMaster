...
import india.chat.demokotlin.data.network.model.CategoryProduct.Otherproducts
import org.jetbrains.annotations.Nullable

class OtherAdapter(private val mValues: List<Otherproducts>, val listener: FeaturedListListner) :
    RecyclerView.Adapter<OtherAdapter.ViewHolder>() {


    private var listListner: OtherAdapter.FeaturedListListner? = null
    internal lateinit var ctx: Context



    interface FeaturedListListner {
        fun onItemClickOtherAdapter(sCatId: String, pId: String)
    }
    ...
    inner class ViewHolder(view: View) : RecyclerView.ViewHolder(view) {
        ...
        open fun bindView(context: Context, listener: FeaturedListListner)
        {
           ...
           llMain!!.setOnClickListener(View.OnClickListener { listener.onItemClickOtherAdapter(tvCartId!!.text.toString(), tvProductId!!.text.toString())})
        }
