...
import india.chat.demokotlin.data.network.model.CategoryProduct.Hotsellingproducts
import org.jetbrains.annotations.Nullable

class HotSealingAdapter(private val mValues: List<Hotsellingproducts>, val listener: FeaturedListListner) :
    RecyclerView.Adapter<HotSealingAdapter.ViewHolder>() {


    private var listListner: HotSealingAdapter.FeaturedListListner? = null
    internal lateinit var ctx: Context



    interface FeaturedListListner {
        fun onItemClickHotsealing(sCatId: String, pId: String)
    }


    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): HotSealingAdapter.ViewHolder {
        ctx = parent.context
    ...
    inner class ViewHolder(view: View) : RecyclerView.ViewHolder(view) {
        ...
        open fun bindView(context: Context, listener: FeaturedListListner)
        {
            ...
            llMain!!.setOnClickListener(View.OnClickListener { listener.onItemClickHotsealing(tvCartId!!.text.toString(), tvProductId!!.text.toString())})
        }
